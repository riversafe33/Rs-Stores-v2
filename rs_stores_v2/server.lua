...
AddEventHandler("rs_stores:getItemsForUI", function()
    ...
    -- Función para añadir categorías en orden y como array
    local function addCategories(list, sourceCategories)
        for _, cat in ipairs(sourceCategories) do
            -- Evitar duplicados
            local existe = false
    ...
    -- Procesar items de compra
    if Config.mostrarComprar then
        for _, item in ipairs(Config.ItemsToBuy) do
        ...
        if Config.categoriasComprar then
            addCategories(categoriasComprar, Config.categoriasComprar)
        end
    end
    ...
    if Config.mostrarVender then
        ...
        for _, item in ipairs(Config.ItemsToSell) do
            local hasItem = false
            local cantidad = 0
        ...
        if Config.categoriasVender then
            addCategories(categoriasVender, Config.categoriasVender)
        end
    end
...
AddEventHandler("rs_stores:getLocalShopItems", function(tiendaId)
    ...
    -- Función para añadir categorías únicas respetando el orden
    local function addUniqueCategory(list, cat)
        for _, v in ipairs(list) do
    ...
    -- Si el config ya tiene categorias definidas, inicialízalas así (mantiene el orden)
    if tienda.categoriasComprar then
        for _, cat in ipairs(tienda.categoriasComprar) do
            addUniqueCategory(categoriasComprar, cat)
        end
    end

    if tienda.categoriasVender then
        for _, cat in ipairs(tienda.categoriasVender) do
            addUniqueCategory(categoriasVender, cat)
        end
    end
    ...
    if tienda.mostrarComprar then
        for _, item in pairs(tienda.ItemsToBuy) do
            ...
            if item.categoria and item.categoria ~= false then
                addUniqueCategory(categoriasComprar, item.categoria)
            end
        end
    end
    ...
    if tienda.mostrarVender then
        ...
        for _, item in pairs(tienda.ItemsToSell) do
            ...
            if hasItem then
                ...
                if item.categoria and item.categoria ~= false then
                    addUniqueCategory(categoriasVender, item.categoria)
                end
            end
        end
    end

    -- Ya no hacemos table.sort para mantener el orden del config
    ...
    if itemData.weapon then
        ...
        if itemData.gold then
            if gold >= totalPrice then
                ...
                TriggerClientEvent("vorp:TipBottom", src, Config.Textos.Notify.buy .. " " .. cantidad .. " x " .. itemData.label .. " " .. Config.Textos.Notify.ford .. " " .. totalPrice .. " " .. Config.Textos.Notify.gold, 4000)
            else
                TriggerClientEvent("vorp:TipBottom", src, Config.Textos.Notify.notgold, 4000)
            end
        else
            if money >= totalPrice then
                ...
                TriggerClientEvent("vorp:TipBottom", src, Config.Textos.Notify.buy .. " " .. cantidad .. " x " .. itemData.label .. " " .. Config.Textos.Notify.ford .. " " .. totalPrice .. "$", 4000)
            else
                TriggerClientEvent("vorp:TipBottom", src, Config.Textos.Notify.notmoney, 4000)
            end
        end
        ...
        if not canCarry then
            TriggerClientEvent("vorp:TipBottom", src, Config.Textos.Notify.notspace, 4000)
            return
        end
