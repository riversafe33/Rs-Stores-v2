...
-- Thread 1: NPC de tienda (ya tenía wait adaptativo, pequeña mejora)
Citizen.CreateThread(function()
    while true do
        ...
        if Config.TiendaNPCActiva and npcshop and DoesEntityExist(npcshop) then
            if #(playerCoords - GetEntityCoords(npcshop)) < 2.0 then
                nearNPC = true
                ...
                -- Loop rápido mientras esté cerca del NPC
                while true do
        ...
        Citizen.Wait(500) -- lejos del NPC: solo 2 checks/s
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(2000) -- más frecuente para distancia
...
Citizen.CreateThread(function()
    while true do
        ...
        -- Fase 1 (500ms): ¿hay alguna tienda cerca? (radio amplio, barato)
        for id, datos in pairs(tiendaPrompts) do
            if #(playerCoords - datos.coords) < 10.0 then
                nearbyId = id
                break
            end
        end
        ...
        if nearbyId then
            -- Fase 2 (0ms): estamos cerca, activar prompt frame a frame
            while true do
                playerCoords = GetEntityCoords(PlayerPedId())
                local stillNear = false

                for id, datos in pairs(tiendaPrompts) do
                    local dist   = #(playerCoords - datos.coords)
                    local tienda = Config.TiendasLocales[id]

                    if dist < 10.0 then stillNear = true end

                    if dist < 2.0 and IsStoreOpen(tienda) then
                ...
                if not stillNear then break end -- salir al loop lento
                Citizen.Wait(0)
            end
        end

        Citizen.Wait(500) -- lejos de toda tienda: solo 2 checks/s
    end
end)
